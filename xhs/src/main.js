import './compat.js';
import {releaseSceneResources} from './scene-lifetime.js';
import {createLandscapeLayout} from './screen-layout.js';
...
let visited=new Set();try{visited=new Set(JSON.parse(localStorage.getItem('mole-xhs-scenes-visited')||'[]').filter(id=>places[id]&&id!=='island'));}catch{}
const view='classic';
let sceneLoading=false,queuedScene=null;
...
const layout=createLandscapeLayout(document.getElementById('app-shell'));
const music=createBackgroundMusic({buttonId:'sound-button',volume:0.55,onStateChange:playing=>{sound=playing;if(ui)ui.setSound(sound);},onError:()=>{if(ui)ui.toast('背景音乐暂时无法播放，请点喇叭再试一次。');}});
const uiOptions={onVisit:visit,onIsland:()=>visit('island'),onReset:resetView,onSound:()=>music.toggle()};
...
function batch(root,dynamic=[]){
 ...
 root.traverse(o=>{
  ...
  if(o.isMesh&&!Array.isArray(o.material)&&!o.material.transparent){
   ...
   let g=o.geometry.clone();if(!g.index){const count=g.attributes.position.count,indices=count>65535?new Uint32Array(count):new Uint16Array(count);for(let i=0;i<count;i++)indices[i]=i;g.setIndex(new THREE.BufferAttribute(indices,1));}g.applyMatrix4(o.matrixWorld);a.geos.push(g);remove.push(o);
 ...
 const released=new Set();remove.forEach(o=>{if(o.geometry&&!retained.has(o.geometry)&&!released.has(o.geometry)){o.geometry.dispose();released.add(o.geometry);}});
 function prune(parent){parent.children.slice().forEach(child=>{if(excluded.has(child))return;prune(child);if(child.isGroup&&!child.children.length)parent.remove(child);});}prune(root);
}

function discardCurrentScene(){
 const roots=[];
 if(current){scene.remove(current.root);roots.push(current.root);}
 portals.forEach(portal=>{scene.remove(portal.object);roots.push(portal.object);});
 const background=scene.background;scene.background=null;
 if(background&&background.isTexture)background.dispose();
 releaseSceneResources(scene,roots);
 current=null;currentId=null;portals=[];cache.clear();
...
function init(){
 ...
 renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.35));renderer.outputColorSpace=THREE.SRGBColorSpace;
 ...
 rig=createCameraRig(canvas,()=>current,{toLocal:layout.toLocal,canInteract:()=>!mapOpen&&!sceneLoading&&!ui.isOverlayOpen(),onTap:walkAtCanvasPoint});camera=rig.camera;controls=rig.controls;
 ...
 if(window.ResizeObserver)new window.ResizeObserver(resize).observe(stage);
 layout.onResize(()=>{if(rig)rig.cancelGesture();resize();});addEvents();
 ui.setVisited([...visited]);ui.setLocationProgress(visited.size);
 window.__manor={ready:true,getState:()=>({layout:layout.getState(),webgl:renderer.capabilities.isWebGL2?'webgl2':'webgl',memory:{...renderer.info.memory},mapOpen,destination:currentId,view,visited:[...visited],player:player.position.toArray(),playerModel:player.userData.modelId,playerYaw:player.rotation.y,moving:moving.length>0,portals:portals.map(p=>({target:p.target,position:p.pos.toArray()})),calls:renderStats.calls,triangles:renderStats.triangles,camera:camera.position.toArray(),projection:camera.type,fov:camera.fov,zoom:camera.zoom,assetsPending:pendingAssets,target:controls.target.toArray(),sceneCount:cache.size,sound,music:music.getState()}),projectClient:(x,y,z)=>{const p=project(new THREE.Vector3(x,y,z));return {...layout.toClient(p.x,p.y,canvas),visible:p.visible};},getRoutes:()=>Object.fromEntries(Object.entries(places).map(([id,p])=>[id,p.links])),project:(x,y,z)=>project(new THREE.Vector3(x,y,z)),canWalk,walkHeight};
 requestAnimationFrame(animate);
 const initialId=location.hash.slice(1);visit(places[initialId]?initialId:'island');
 }catch(error){console.error(error);ui.setLoading(false);ui.showError('暂时无法显示 3D 画面，请重新进入或换一台支持 3D 的设备。');}
}
function resize(){if(!renderer)return;const w=stage.clientWidth,h=stage.clientHeight;if(!w||!h)return;renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.35,Math.sqrt(1200000/(w*h))));renderer.setSize(w,h,false);const size=renderer.getDrawingBufferSize(new THREE.Vector2());painter.resize(size.x,size.y);rig.size(w,h);}
function resetView(){if(current)rig.reset();}

// Wait for the reference-derived sign textures before showing a rebuilt scene.
let pendingAssets=0,assetWaiters=[],assetFailures=[];
...
manager.itemError=url=>assetFailures.push(url);
const waitForAssets=()=>new Promise((resolve,reject)=>{
 const complete=()=>{clearTimeout(timer);assetFailures.length?reject(new Error('场景素材没有加载成功')):resolve();};
 const timer=setTimeout(()=>reject(new Error('场景素材加载超时')),15000);
 if(pendingAssets)assetWaiters.push(complete);else complete();
});
...
async function enterScene(id,from){
 if(sceneLoading){queuedScene={id,from};return;}sceneLoading=true;assetFailures=[];
 ...
 try{
  ...
  if(currentId!==id){discardCurrentScene();await new Promise(resolve=>requestAnimationFrame(resolve));if(ticket!==transition)return;}
  if(!cache.has(id)){const next=places[id].make();next.dynamic=next.dynamic||[];painter.prepare(next.root,id==='castle');batch(next.root,next.dynamic);cache.set(id,next);}await waitForAssets();if(ticket!==transition){const abandoned=cache.get(id);if(abandoned!==current){releaseSceneResources(scene,[abandoned.root]);cache.delete(id);}return;}
  if(current)scene.remove(current.root);portals.forEach(p=>{scene.remove(p.object);p.object.traverse(o=>o.geometry?.dispose());});
  current=cache.get(id);currentId=id;scene.add(current.root);
  renderer.renderLists.dispose();scene.background=current.indoor?new THREE.Color(current.background||0xb4a69a):id==='island'?new THREE.Color(0x459ac0):referenceSky(id);
  ...
  if(id!=='island')visited.add(id);try{localStorage.setItem('mole-xhs-scenes-visited',JSON.stringify([...visited]));}catch{}
  ...
  try{if(visited.size===Object.keys(places).length-1&&!sessionStorage.getItem('mole-xhs-all-seen')){ui.toast('每一处风景，都留下了你的脚印。');sessionStorage.setItem('mole-xhs-all-seen','1');}}catch(ignore){}
 }catch(error){console.error(error);const failed=cache.get(id);if(failed&&failed!==current){releaseSceneResources(scene,[failed.root]);cache.delete(id);}queuedScene=null;ui.setLoading(false);ui.showError('场景没有加载成功，请点“重新进入”重试。');}
 finally{sceneLoading=false;const next=queuedScene;queuedScene=null;if(next)enterScene(next.id,next.from);}
}
...
function walkAtCanvasPoint(point){
 if(mapOpen||!current||ui.isOverlayOpen())return;
 pointer.set(point.x/canvas.clientWidth*2-1,-point.y/canvas.clientHeight*2+1);
 raycaster.setFromCamera(pointer,camera);
 const at=pickWalkSurface(raycaster.ray,current.walkLevels,walkHeight,canWalk);
 if(at)moveTo(at.x,at.z);
}
function addEvents(){
 addEventListener('hashchange',()=>{const id=location.hash.slice(1);visit(places[id]?id:'island');});
 addEventListener('keydown',e=>{if(['INPUT','TEXTAREA'].includes(document.activeElement&&document.activeElement.tagName)||ui.isOverlayOpen())return;if(!mapOpen&&['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','ShiftLeft','ShiftRight'].includes(e.code)){keys.add(e.code);e.preventDefault();}});
 addEventListener('keyup',e=>keys.delete(e.code));addEventListener('blur',()=>{keys.clear();rig.cancelGesture();});document.addEventListener('visibilitychange',()=>{keys.clear();rig.cancelGesture();last=performance.now();});
 canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();ui.showError('画面暂时休息了，点“重新进入”继续游览。');});
}
let shadowTime=-1,renderTime=0;
function animate(now){
 requestAnimationFrame(animate);if(now-renderTime<32)return;renderTime=now;const dt=Math.min((now-last)/1000,.18);last=now;if(document.hidden||mapOpen||!current)return;time+=dt;
 const paused=ui.isOverlayOpen()||sceneLoading;if(paused)return;
 ...
 if(time-shadowTime>.4){sun.shadow.needsUpdate=true;shadowTime=time;}
